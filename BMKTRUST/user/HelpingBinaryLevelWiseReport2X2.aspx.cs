...
public partial class user_HelpingBinaryLevelWiseReport2X2 : System.Web.UI.Page
{
    ...
    void LoadReport(string userId)
    {
        ...
        var counts = dt.AsEnumerable()
            ...
            .GroupBy(r => ToInt(r["userlevel"]))
            .ToDictionary(g => g.Key, g => g.Count());

        // Always list the full matrix depth so the pending levels show their target too
        int deepest = counts.Count > 0 ? counts.Keys.Max() : 0;
        int lastLevel = Math.Max(deepest, MatrixDepth);

        var levels = Enumerable.Range(1, lastLevel)
            .Select(level =>
            {
                int completed = counts.ContainsKey(level) ? counts[level] : 0;
                long required = 1L << Math.Min(level, 62);
