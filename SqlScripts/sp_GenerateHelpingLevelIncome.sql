-- Level Closing — HelpingBinaryDetail → HelpingLevelIncomeDetail (+ Pool2 entry on Level 1)
-- @FromDate / @ToDate: used as MentionDate on inserts (closing UI)
-- Team count = full downline (original). Pool2 entry also backfills if Level-1 income exists but Pool2 row missing.
IF OBJECT_ID('dbo.sp_GenerateHelpingLevelIncome', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_GenerateHelpingLevelIncome;
GO

CREATE PROC sp_GenerateHelpingLevelIncome
    @FromDate DATETIME,
    @ToDate DATETIME
AS
SET NOCOUNT ON
...
BEGIN
    ...
    BEGIN
        ...
        FROM MyCTE
        WHERE MyCTE.userlevel >= 1 AND MyCTE.userlevel <= 4
        ...
        DECLARE @parentid INT, @sponserid NVARCHAR(100), @sponsercount INT, @StandingPositionHelping NVARCHAR(100)

        SET @maxlevel = (SELECT MAX(levelno) FROM HelpingLevelDetail WITH (NOLOCK))
        SET @levelno = 1
        WHILE (@levelno <= @maxlevel)
        BEGIN
            SET @target = ISNULL((SELECT target FROM HelpingLevelDetail WITH (NOLOCK) WHERE LevelNo = @levelno), 0)
            SET @finalamount = ISNULL((SELECT finalamount FROM HelpingLevelDetail WHERE levelno = @levelno), 0)
            SET @team = ISNULL((SELECT totalteam FROM @tempteam WHERE levelno = @levelno), 0)

            IF (@team = @target)
            BEGIN
                IF (NOT EXISTS (
                    SELECT helpingid FROM HelpingLevelIncomeDetail WITH (NOLOCK)
                    WHERE helpingid = @HelpingId AND levelno = @levelno
                ))
                BEGIN
                    INSERT INTO HelpingLevelIncomeDetail (HelpingId, UserId, LevelNo, Income, MentionBy, MentionDate)
                    VALUES (@HelpingId, @userid, @levelno, @finalamount, 'admin', @ClosingDate)
            ...
            --========================== Pool 2 Entry (Level 1) =====================
            -- Runs even if Level-1 income already exists (backfill missing Pool2 rows)
            IF (@levelno = 1
                AND EXISTS (
                    SELECT helpingid FROM HelpingLevelIncomeDetail WITH (NOLOCK)
                    WHERE helpingid = @HelpingId AND levelno = 1
                )
                AND NOT EXISTS (
